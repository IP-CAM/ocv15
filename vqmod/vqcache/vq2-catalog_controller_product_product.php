...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
				...
				$this->data['group_products'] = array();
				...
				if($product_info['group_model']) {
					...
					foreach($group_products as $kk=>$group_product) {
						$this->data['group_products'][$kk]['model'] = $group_product['model'];
						$this->data['group_products'][$kk]['model_tag'] = $group_product['model_tag'];
						$this->data['group_products'][$kk]['chk_price'] = $group_product['chk_price'];
						$this->data['group_products'][$kk]['chk_image'] = $group_product['chk_image'];
						$this->data['group_products'][$kk]['chk_name'] = $group_product['chk_name'];
						$this->data['group_products'][$kk]['href'] = $this->url->link('product/product', 'product_id=' . $group_product['product_id']);

						if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
							$this->data['group_products'][$kk]['price'] = $this->currency->format($this->tax->calculate($group_product['price'], $group_product['tax_class_id'], $this->config->get('config_tax')));
						} else {
							$this->data['group_products'][$kk]['price'] = false;
						}
									
						if ((float)$group_product['special']) {
							$this->data['group_products'][$kk]['price'] = $this->currency->format($this->tax->calculate($group_product['special'], $group_product['tax_class_id'], $this->config->get('config_tax')));
						}

						if ($group_product['image']) {
							$this->data['group_products'][$kk]['image'] = $this->model_tool_image->resize($group_product['image'], 48, 48);
						} else {
							$this->data['group_products'][$kk]['image'] = '';
						}
					}
				}
