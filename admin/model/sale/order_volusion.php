<?php
class ModelSaleOrderVolusion extends Model {
	public function addColumns() {
		// Shipped Date field
		$query = $this->db->query("SHOW COLUMNS FROM " . DB_PREFIX . "order WHERE field = 'date_shipped'");
		$exists = 0;
		foreach ($query->rows as $index) {$exists++;}
		if (!$exists) {$this->db->query("ALTER TABLE " . DB_PREFIX . "order ADD COLUMN `date_shipped` DATETIME NULL;");}

		// Private Notes field
		$query = $this->db->query("SHOW COLUMNS FROM " . DB_PREFIX . "order WHERE field = 'internal_comment'");
		$exists = 0;
		foreach ($query->rows as $index) {$exists++;}
		if (!$exists) {$this->db->query("ALTER TABLE " . DB_PREFIX . "order ADD COLUMN `internal_comment` text NULL;");}

		// Gift Notes field
		$query = $this->db->query("SHOW COLUMNS FROM " . DB_PREFIX . "order WHERE field = 'gift_comment'");
		$exists = 0;
		foreach ($query->rows as $index) {$exists++;}
		if (!$exists) {$this->db->query("ALTER TABLE " . DB_PREFIX . "order ADD COLUMN `gift_comment` text NULL;");}

		// Customer Account Notes field
		$query = $this->db->query("SHOW COLUMNS FROM " . DB_PREFIX . "customer WHERE field = 'comment'");
		$exists = 0;
		foreach ($query->rows as $index) {$exists++;}
	...
	public function saveOrder($order_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$data['order_status_id'] . "', comment = '" . $this->db->escape($data['comment']) . "', internal_comment = '" . $this->db->escape($data['internal_comment']) . "', gift_comment = '" . $this->db->escape($data['gift_comment']) . "' WHERE order_id = '" . (int)$order_id . "'");

		if($data['customer_id'] > 0) {
			$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET comment = '" . $this->db->escape($data['customer_comment']) . "' WHERE customer_id = '" . (int)$data['customer_id'] . "'");
		}
	...
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT *, (SELECT CONCAT(c.firstname, ' ', c.lastname) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = o.customer_id) AS customer, (SELECT c2.comment FROM " . DB_PREFIX . "customer c2 WHERE c2.customer_id = o.customer_id) AS customer_comment FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'");
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'date_shipped'			  => $order_query->row['date_shipped'],
				'internal_comment'        => $order_query->row['internal_comment'],
				'gift_comment'			  => $order_query->row['gift_comment'],
				'customer_comment'        => $order_query->row['customer_comment']
			);
		} else {
			return false;
		}
